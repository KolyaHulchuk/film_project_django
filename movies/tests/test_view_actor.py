...
def test_popular_actor(client, tmdb, value):
    response = client.get("/movies/popular-actors/")

    assert response.status_code == 200
    assert "actors" in response.context
    assert response.context["actors"][0]["known_for_department"] == "Acting"
    assert len(response.context["actors"]) == 1
    assert response.context["actors"][0]["name"] == "Ana de Armas"
